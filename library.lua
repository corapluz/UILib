	...
	do 
		...
		Library.Window = function(self, Data)
			...
			local Items = { } do
				...
				local LeftTabsOriginalTransparencies = {}

				local function SetEdgesEnabled(Enabled)
					for _, Child in Items["MainFrame"].Instance:GetChildren() do
						if Child:IsA("TextButton") and Child.ZIndex == 99999 then
				...
				Items["MinimizeButton"]:Connect("MouseButton1Down", function()
					...
					local SlideTime = TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
					local CollapseTime = TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)

					local function TweenChildren(Parent, FadingOut)
						for _, Child in Parent:GetDescendants() do
							if Child:IsA("TextLabel") or Child:IsA("TextButton") or Child:IsA("TextBox") then
								if FadingOut then
									LeftTabsOriginalTransparencies[Child] = Child.TextTransparency
									TweenService:Create(Child, SlideTime, {TextTransparency = 1}):Play()
								else
									TweenService:Create(Child, SlideTime, {TextTransparency = LeftTabsOriginalTransparencies[Child] or 0}):Play()
								end
							elseif Child:IsA("ImageLabel") or Child:IsA("ImageButton") then
								if FadingOut then
									LeftTabsOriginalTransparencies[Child] = Child.ImageTransparency
									TweenService:Create(Child, SlideTime, {ImageTransparency = 1}):Play()
								else
									TweenService:Create(Child, SlideTime, {ImageTransparency = LeftTabsOriginalTransparencies[Child] or 0}):Play()
								end
							elseif Child:IsA("Frame") or Child:IsA("ScrollingFrame") then
								if FadingOut then
									LeftTabsOriginalTransparencies[Child] = Child.BackgroundTransparency
									TweenService:Create(Child, SlideTime, {BackgroundTransparency = 1}):Play()
								else
									TweenService:Create(Child, SlideTime, {BackgroundTransparency = LeftTabsOriginalTransparencies[Child] or 1}):Play()
								end
							end
						end
					end
					...
					if IsMinimized then
						OriginalHeight = Items["MainFrame"].Instance.AbsoluteSize.Y
						OriginalWidth  = Items["MainFrame"].Instance.AbsoluteSize.X

						SetEdgesEnabled(false)

						Items["MainFrame"].Instance.ClipsDescendants = true
						Items["LeftTabs"].Instance.ClipsDescendants = true

						TweenChildren(Items["LeftTabs"].Instance, true)
						Items["LeftTabs"]:Tween(SlideTime, {
							Size = UDim2New(0, 0, 1, 0),
							BackgroundTransparency = 1
						})
						...
						Items["Content"]:Tween(CollapseTime, {
							Size = UDim2New(1, 0, 0, 0),
							BackgroundTransparency = 1
						})
						Items["MainFrame"]:Tween(CollapseTime, {
							Size = UDim2New(0, OriginalWidth, 0, 55)
						})
						...
						Items["MainFrame"]:Tween(CollapseTime, {
							Size = UDim2New(0, OriginalWidth, 0, OriginalHeight or 644)
						})
						Items["Content"]:Tween(CollapseTime, {
							Size = UDim2New(1, 0, 1, -55),
							BackgroundTransparency = 0.75
						...
						TweenChildren(Items["LeftTabs"].Instance, false)

						task.wait(0.35)

						SetEdgesEnabled(true)

						Items["MainFrame"].Instance.ClipsDescendants = false
...
Library.Sections.Dropdown = function(self, Data)
    ...
    0, Items["RealDropdown"].Instance.AbsolutePosition.Y + Items["RealDropdown"].Instance.AbsoluteSize.Y + 5
		...
		for _, Value in Descendants do
			...
			if not Value.ClassName:find("UI") then
				Value.ZIndex = (Dropdown.IsOpen and Dropdown.Section.IsSettings and 8)
				           or  (Dropdown.IsOpen and 3)
				           or  1
			end
	...
	function Dropdown:Add(Option)
		...
		Instances:Create("UICorner", {Parent = OptionAccent.Instance, Name = "\0"})
	...
	Items["RealDropdown"]:Connect("Changed", function(Property)
		if Property == "AbsolutePosition" and Dropdown.IsOpen then
			Dropdown.IsOpen = not Library:IsClipped(
				Items["OptionHolder"].Instance,
				Dropdown.Section.Items["Section"].Instance.Parent
			)
			Items["OptionHolder"].Instance.Visible = Dropdown.IsOpen
		end
	end)
